...
class PythonController:
    ...
    def execute_action(self, action: Dict[str, Any]):
        ...
        if action_type == "MOVE_TO":
            ...
            # Получаем платформу
            try:
                platform_info = self.get_vm_platform()
                is_macos = platform_info and "Darwin" in platform_info
            except:
                # Если не удалось определить платформу, используем универсальный подход
                is_macos = False
                logger.warning(
                    "Could not determine platform, defaulting to non-macOS behavior"
                )

            # Нормализуем модификаторы для macOS
            def normalize_modifier_for_macos(key):
                key_lower = key.lower()
            ...
            if is_macos and any(
                key.lower() in ["ctrl", "control", "cmd", "command"] for key in keys
            ):
                # Для macOS с модификаторами используем with pyautogui.hold()
                modifier_keys = []
                main_keys = []

                # Разделяем на модификаторы и основные клавиши
                for key in keys:
                    key_lower = key.lower()
                    if key_lower in [
                        "ctrl",
                        ...
                        modifier_keys.append(normalize_modifier_for_macos(key))
                    else:
                        main_keys.append(key)

                if not main_keys:
                    # Если основных клавиш нет, используем последнюю
                    main_keys = [keys[-1]]
                    modifier_keys = keys[:-1]

                # Создаем команду для macOS
                modifier_keys_str = json.dumps(modifier_keys)

                if len(main_keys) == 1:
                    main_key_str = json.dumps(main_keys[0])
                    command = f"""
import time
import pyautogui
with pyautogui.hold({modifier_keys_str}):
    time.sleep(0.1)
    pyautogui.press({main_key_str})
        """
                else:
                    # Если несколько основных клавиш, нажимаем их последовательно
                    main_keys_str = json.dumps(main_keys)
                    command = f"""
import time
import pyautogui
with pyautogui.hold({modifier_keys_str}):
    time.sleep(0.1)
    for key in {main_keys_str}:
        pyautogui.press(key)
        time.sleep(0.05)
        """
                ...
                # Для Windows/Linux или простых комбинаций используем pyautogui.hotkey()
